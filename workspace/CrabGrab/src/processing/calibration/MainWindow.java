 ...
 * @author Nicolas Wenk
 *
 *         This class is used to calibrate the main application with a
 *         chessboard.
 *
 */
public class MainWindow extends PApplet
{
	...
	ArrayList<PVector> drawChessboard(int x0, int y0, int cwidth)
	{
		...
		if (calibrated)
		{
			for (PVector p : testingPoints)
			{
				...
				Core.perspectiveTransform(matSrc, matDest, H);
	...
	public void mousePressed()
	{
		if (mouseX < wWidth - sidePanelWidth)
		{
			if (!calibrated)
			{
				...
				System.out.println("Hey " + mouseX + " => "
						+ clickedPoints.get(clickedPoints.size() - 1));
			} else
			{
	...
	void calibrate()
	{
		...
		H = Calib3d.findHomography(scene, obj);
