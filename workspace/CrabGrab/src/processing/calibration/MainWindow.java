...
package processing.calibration;
...
public class MainWindow extends PApplet
{
	...
	void setupGui()
	{
		...
		guiCpos = cp5.addSlider2D("chessPosition").setLabel("Position")
				.setPosition(guiPos.x, guiPos.y + 15)
				.setSize(sidePanelWidth,
						(int) (sidePanelWidth * (wHeight
								/ (float) (wWidth - sidePanelWidth))))
				.setArrayValue(new float[]{0, 0});

		guiCwidth = cp5.addSlider("cwidth")
				.setPosition(guiPos.x, guiPos.y + 420).setHeight(30)
				.setWidth(sidePanelWidth).setRange(5, 800).setValue(100)
				.setLabel("Size");

		guiCalibrate = cp5.addButton("calibrate")
				.setPosition(guiPos.x, guiPos.y + 600)
				.setSize(sidePanelWidth, 32).addCallback(new CallbackListener()
				{
	...
	public void draw()
	{
		...
		{
			fill(255, 255, 0);
			ellipse(map(p.x, 0, kinect2.depthWidth, 0, wWidth - sidePanelWidth),
					map(p.y, 0, kinect2.depthHeight, 0, wHeight), 5, 5);
		}
	}
	...
	ArrayList<PVector> drawChessboard(int x0, int y0, int cwidth)
	{
		...
		for (int j = 0; j < 4; j++)
		{
			for (int i = 0; i < 5; i++)
			{
				...
				if (i > 0 && j > 0)
				{
					projPoints.add(new PVector((float) x / pWidth,
							(float) y / pHeight));
				}
				if ((i + j) % 2 == 0)
				{
		...
		ca.fill(0, 255, 0);
		/*
		 * if (calibrated) { ca.ellipse(testPointP.x, testPointP.y, 20, 20); }
		 */
		...
		if (calibrated)
		{
			for (PVector p : testingPoints)
			{
				...
				ca.fill(0, 255, 0);
				System.out.println(
						p.x + ", " + p.y + " => " + (int) matDest.get(0, 0)[0]
								+ ", " + (int) matDest.get(0, 0)[1]);
				ca.ellipse((int) (matDest.get(0, 0)[0] * pWidth),
						(int) (matDest.get(0, 0)[1] * pHeight), 5, 5);
			}
		}
		ca.redraw();
		return projPoints;
	...
	public void mousePressed()
	{
		if (mouseX < wWidth - sidePanelWidth)
		{
			if (!calibrated)
			{
				clickedPoints.add(new PVector(
						map(mouseX, 0, wWidth - sidePanelWidth, 0,
								kinect2.depthWidth),
						map(mouseY, 0, wHeight, 0, kinect2.depthHeight)));
			} else
			{
				testingPoints.add(new PVector(
						map(mouseX, 0, wWidth - sidePanelWidth, 0,
								kinect2.depthWidth),
						map(mouseY, 0, wHeight, 0, kinect2.depthHeight)));
			}
	...
	void calibrate()
	{
		...
		List<Point> listClickedPoints = new ArrayList<Point>(
				clickedPoints.size());
		for (PVector p : clickedPoints)
		{
			Point point = new Point(p.x, p.y);
